...
final class PackagingTests: XCTestCase {
    ...
    func testScaleClamp() {
        ...
        XCTAssertEqual(scale.clamp(11.0, within: 5 ... 10.0), 11.0)
        XCTAssertEqual(scale.clamp(1.0, within: 5 ... 10.0), 1.0)
        XCTAssertEqual(scale.clamp(7.0, within: 5 ... 10.0), 7.0)

        let cScale = LinearScale(domain: 5 ... 10.0, isClamped: true)
        XCTAssertEqual(cScale.clamp(11.0, within: 5 ... 10.0), 10.0)
        XCTAssertEqual(cScale.clamp(1.0, within: 5 ... 10.0), 5.0)
        XCTAssertEqual(scale.clamp(7.0, within: 5 ... 10.0), 7.0)
    }
}
